package com.forgestorm.topdown.camera;
...
public class TkPerspectiveCameraController extends GestureDetector {
    ...
    public int downKey = Keys.SHIFT_LEFT;
    protected boolean downPressed;
    public int rotateRightKey = -1;
    protected boolean rotateRightPressed;
    public int rotateLeftKey = -1;
    ...
    protected static class CameraGestureListener extends GestureAdapter {
        public TkPerspectiveCameraController controller;
    ...
    protected TkPerspectiveCameraController(final CameraGestureListener gestureListener, final Camera camera) {
    ...
    public TkPerspectiveCameraController(final Camera camera) {
    ...
    public void update() {
        if (rotateRightPressed || rotateLeftPressed || forwardPressed || backwardPressed || leftPressed || rightPressed || upPressed || downPressed) {
            ...
            if (leftPressed) {
                ...
                if (translateTarget) target.add(tmpV1);
            }
            if (rightPressed) {
                camera.translate(tmpV1.set(camera.direction).crs(camera.up).nor().scl(delta * translateUnits));
                if (translateTarget) target.add(tmpV1);
            ...
            if (upPressed) {
                camera.translate(tmpV2.set(camera.up).scl(delta * translateUnits));
                if (translateTarget) target.add(tmpV2);
            }
            if (downPressed) {
                camera.translate(tmpV2.set(camera.up).scl(-delta * translateUnits));
                if (translateTarget) target.add(tmpV2);
